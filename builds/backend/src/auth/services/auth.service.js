...
let AuthService = class AuthService {
    ...
    async validateUser(loginDto) {
        ...
        if (user.isBlocked) {
            common_1.Logger.log(`User is blocked userId: ${user.userId} email: ${user.email}`);
            throw new common_1.HttpException('You are blocked :(', 423);
        }
    ...
    async register(createUserDto) {
        ...
        try {
            ...
            if (e.code === prisma_error_enum_1.PrismaError.UniqueConstraintViolation) {
                common_1.Logger.log(`Email already in use ${createUserDto.email}`);
